...
##
##    **Internal function rpartmvcallback**
##
## **************************************************************************
##

rpartmvcallback <- function(mlist, nobs, init) {
  if(length(mlist) < 3L)
  ...
  if(numy == 1L) {
    ...
    expr1 <- quote({
      if(nback < 0L) { #categorical variable
        n2 <- -1*nback
        temp  <- user.split(yback[1:n2], wback[1L:n2],
    ...
    expr1 <- quote({
      if(nback < 0L) { #categorical variable
        ...
        temp <- user.split(tempy, wback[1L:nback],xback[1L:nback],
                           parms, TRUE)
