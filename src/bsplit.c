...
void bsplit(
  ...
  */
  ...
  for(i = 0; i < rp.nvar; i++) {
    ...
    if(rp.method == 5) {
      if(!(index[0] >= 0 && which[index[0]] == nodenum))
        k = 0;
      else
        k = 1;
      for(j = 1; j < rp.n; j++)
        if(index[j] >= 0 && which[index[j]] == nodenum) {
          ...
          for(jj = 0; jj < j; jj++)
            if(index[jj] >=0 && which[index[jj]]== nodenum) {
              ...
              ytemp[rp.n*kk - kk*(kk + 1)/2 + k - kk - 1] =
                rp.ydata[rp.n*ijj - ijj*(ijj + 1)/2 + ij - ijj - 1]; 
              kk++;
            }
          k++;
        }
    ...
    (*rp_choose)(k, ytemp, xtemp, nc, rp.min_node, &improve, &split, rp.csplit,
     me->risk, wtemp);
