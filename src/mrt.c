  ...
  if(nclass == 0) {
    ...
    for(i = 0; right_n > edge; i++) {
      ...
      for(j = 0; j < rp.num_y; j++) {
        ...
        tdiffs[j] += diffs[j];
        if(rp.dissim == 1)
          sumdiffs_sq += tdiffs[j]*tdiffs[j];
        else if(rp.dissim == 2)
          sumdiffs_sq += fabs(tdiffs[j]);
      }
      ...
      if((x[i + 1] != x[i]) && (left_n >= edge)) {
        if(rp.dissim == 1)
          temp = sumdiffs_sq/left_wt + sumdiffs_sq/right_wt;
        else if(rp.dissim == 2)
          temp = 2.0*sumdiffs_sq;
    ...
    *improve = best/myrisk;
    ...
    for(i = 0; i < n; i++) {
      ...
      countwt[k] += wt[i];
      for(j = 0; j < rp.num_y; j++)
        mean[k + nclass*j] += (y[i][j] - grandmean[j])*wt[i];
    }
    
    for(i = 0; i < nclass; i++)
      for(j = 0; j < rp.num_y; j++)
       if (countwt[i] > 0)
         mean[i + nclass*j] /= countwt[i];
    ...
    */
    ...
    while((i = graycode()) < nclass) {
      ...
      if(tsplit[i] == LEFT) {
        ...
        left_wt -= countwt[i];
        for(j = 0; j < rp.num_y; j++)
          tdiffs[j] += mean[i+nclass*j]*countwt[i];
      }
      else {
        tsplit[i] = LEFT;
        right_n -= countn[i];
