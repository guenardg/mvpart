 ...
 * Cross-validates a model. This routine is responsible for filling in
  ...
  */
  for(i = 0; i < n_xval; i++) {
    ...
    */
    ...
    xtree->num_obs = k;
    size = (int)temp;
    (*rp_init)(k, rp.ytemp, maxcat, error, parms, &size, 2, rp.wtemp);
  ...
  for(cplist = cptable_head; cplist != 0; cplist = cplist->forward)
    cplist->xstd = sqrt(cplist->xstd - cplist->xrisk*cplist->xrisk/total_wt);
  rp.alpha = alphasave;
  for(i = 0; i < rp.n; i++)
    rp.which[i] = savew[i];
  Free(savew);
